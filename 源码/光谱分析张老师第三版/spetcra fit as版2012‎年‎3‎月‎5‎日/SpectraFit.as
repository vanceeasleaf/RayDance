package  {
	...
	public class SpectraFit extends MovieClip {
		...
		private var jmax:int=85	    //最大转动能级量子数
		private var  NQ:int=5
		...
		public  function SpectraFit(){trace(h0)
			Txt.read ('H00.txt',Xorg,Yorg,init);//导入数据，完成后执行init
		}
		function init():void {
		...
		function PFun():void{
			...
			for(var  j:int=1;j<=paraNumber;j++){    //拟合参数微分对光谱强度的影响
			Ycal[j+1]=[]
				var DP=Porg[j]*DP0
				P[j]=Porg[j]-DP
				 Spectra()
		...
		function Spectra():void{
			...
			var NRange=PRange/Dstep   	      //发光峰数据点范围
			...
			for( i=1;i<=peaksNumber;i++){
				...
				jmax=85
				if (N1==1) jmax=65		  //Predissociation
				if (N1==2) jmax=55		  //Predissociation
				if (N1==3) jmax=43		  //Predissociation
				if (N1==4) jmax=28		  //Predissociation
				...
				for(var j:int=0;j<=jmax;j++){
					...
					var imax=NWL+NRange
					if (imin<1) imin=1
					if (imax>pointsNumber) imax=pointsNumber
				...
				for(j=1;j<=jmax;j++){
					...
					var PQ=WP*WP*WP*WP*PD*SP	      //计算发射强度
		...
		function Rotation(Lvib1,Lvib2,Rl1,Rl2):void{
			...
			for( var i:int=0;i<=Lvib1;i++){
				F10[i+1]=[]   //注意数组编号比量子数+1
					...
					F12[i+1]=[]
				var V1=Number(i)+0.5
				//   Av1=Ae1-0.00228*V1**2+0.000733*V1**3-0.00015*V1**4
				var B1=Be1-Ae1*V1+Ge1*V1*V1				   //Bv
				//   B1=1.82682-0.0243*V1+0.00192*V1**2-0.00062*V1**3
				//   D1=4.0*B1**3/W01**2					   //Dv
				//   DB=8.0*G1(2)/G1(1)-3.0*Ae1/Be1-Ae1**2*G1(1)/Be1**3/24.0
				//   H1=2.0*D1*(12.0*Be1**2-Ae1*G1(1))/3.0/G1(1)**3  //Hv
				//   D1=D1*(1.0+DB*(V1+0.5))				   //Dv
				...
				for(var j:int=0;j<=99;j++){
					...
					var F3=B1*(Fj+Math.sqrt(ZA1)-2.0*ZB1)-D1*(Rj+1.5)*(Rj+1.5)*(Rj+1.5)*(Rj+1.5) //上2转动能(波数单位)
								//         F10(i+1,j+1)=F1+H1*Fj**3		   //注意数组编号比量子数+1
								//         F11(i+1,j+1)=F2+H1*Fj**3		   //注意数组编号比量子数+1
								//         F12(i+1,j+1)=F3+H1*Fj**3		   //注意数组编号比量子数+1
			...
			for(  i=0;i<=Lvib2;i++){
				...
				// B2=1.637698-0.017861*V2-0.000144*V2**2+1.05e-5*V2**3-4.2e-6*V2**4
				//   D2=4.0*B2**3/W02**2
				//   DB=8.0*G2(2)/G2(1)-3.0*Ae2/Be2-Ae2**2*G2(1)/Be2**3/24.0
				//   H2=2.0*D2*(12.0*Be2**2-Ae2*G2(1))/3.0/G2(1)**3
				//   D2=D2*(1.0+DB*(V2+0.5))
				...
				for( j=0;j<=99;j++){
					...
					F3=B2*(Fj+Math.sqrt(ZA2)-2.0*ZB2)-D2*(Rj+1.5)*(Rj+1.5)*(Rj+1.5)*(Rj+1.5) //下2转动能
								//         F20(i+1,j+1)=F1+H2*Fj**3
								//         F21(i+1,j+1)=F2+H2*Fj**3
								//         F22(i+1,j+1)=F3+H2*Fj**3
					F20[i+1][j+1]=F1
					F21[i+1][j+1]=F2
					F22[i+1][j+1]=F3
					//      endif
...
//			COMMON/Hon3/SP3(5,9,100),SQ3(5,9,100),SR3(5,9,100)
			...
			for(var i1:int=0;i1<=Lvib1;i1++){
				SP1[i1+1]=[];SQ1[i1+1]=[];SR1[i1+1]=[];
				SP2[i1+1]=[];SQ2[i1+1]=[];SR2[i1+1]=[];
				SP3[i1+1]=[];SQ3[i1+1]=[];SR3[i1+1]=[];
				var V1=Number(i1)+0.5
				var B1=Be1-Ae1*V1+Ge1*V1*V1
					var Y1=A1/B1
				for( var  i2:int=0;i2<=Lvib2;i2++){
					SP1[i1+1][i2+1]=[];SQ1[i1+1][i2+1]=[];SR1[i1+1][i2+1]=[];
				SP2[i1+1][i2+1]=[];SQ2[i1+1][i2+1]=[];SR2[i1+1][i2+1]=[];
				SP3[i1+1][i2+1]=[];SQ3[i1+1][i2+1]=[];SR3[i1+1][i2+1]=[];
					...
					for( j=0;j<=90;j++){
						...
						//		 i=j									 //j2=j1
						//		 A=4.0*(G-1)*(G+1)*(G+1)
						//         B=(G+2)*Uq1(j+1)*Uq2(j+1)
						//         C=0.0
						//         if (j>=0) then
						//            D=(G-1)**2*(G+G+1)/4.0/G**3/(G+1)**3/C11(j+1)/C21(j+1)
						//            SQ1(i1+1,i2+1,j+1)=(A+B+C)*(A+B+C)*D
						//		 else
						//            SQ1(i1+1,i2+1,j+1)=0.0
						//         endif
						G=H-1.0								 //j2=j1-1
						i=j-1									 //j2=j1-1
						A=8.0*(G-1)*G*(G+1)*(G+2)
						B=(G-1)*(G+3)*Up1[i+2]*Up2[i+1]
						...
						//write(10,*) j,SP1(i1+1,i2+1,j+1),SR1(i1+1,i2+1,j+1)
						//RR2=SR2(i1+1,i2+1,j+1)
						//write(20,*) j,SP2(i1+1,i2+1,j+1),SQ2(i1+1,i2+1,j+1),RR2
						//RR3=SR3(i1+1,i2+1,j+1)
						//write(30,*) j,SP3(i1+1,i2+1,j+1),SQ3(i1+1,i2+1,j+1),RR3
					}
				}
			}
		}
