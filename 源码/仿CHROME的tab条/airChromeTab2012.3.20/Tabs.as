package {
	...
	public class Tabs extends UIMovieClip {
		var tabs:Array 
		var _activeIndex:int = 0;
		var 交叠:Number = 8;
		var oriWidth:Number 
		var curWidth:Number ;
		var scaleWidth:Number=700;
		var edge=8;
		...
		public function createNewTab(){
			...
			addChild(tab);
			tabs.push(tab);
			tab.active = true;trace(_activeIndex)
		...
		function onTabClose(e:MouseEvent )
		{
			...
			dispatchEvent(new CloseTabEvent(index));
			}
			}
			);
		...
		function activeControl(e:MouseEvent )
		{
			...
			dispatchEvent(new ActiveTabEvent(_activeIndex));
		}
	}
}
